...
def test_gallery_day_creation(test_user):
    ...
    assert gallery_day.user == test_user  # nosec
    assert gallery_day.date == today  # nosec
    assert GalleryDay.objects.count() == 1  # nosec


def test_gallery_day_str_method(test_gallery_day):
    expected_str = f"{test_gallery_day.user.username} - {test_gallery_day.date}"
    assert str(test_gallery_day) == expected_str  # nosec
...
def test_gallery_day_unique_for_different_users(test_user, another_user):
    ...
    assert GalleryDay.objects.count() == 2  # nosec
...
def test_photo_gallery_creation(test_gallery_day, dummy_image_file):
    ...
    assert photo.gallery_day == test_gallery_day  # nosec
    assert photo.description == "A beautiful sunrise"  # nosec
    assert photo.image.name.startswith("photos/test_image_")  # nosec
    assert PhotoGallery.objects.count() == 1  # nosec
...
def test_photo_gallery_no_description(test_gallery_day, dummy_image_file):
    ...
    assert photo.description == ""  # nosec
...
def test_video_gallery_creation(test_gallery_day, dummy_video_file):
    ...
    assert video.gallery_day == test_gallery_day  # nosec
    assert video.description == "A short clip"  # nosec
    assert video.video.name.startswith("videos/test_video_")  # nosec
    assert VideoGallery.objects.count() == 1  # nosec
...
def test_video_gallery_no_description(test_gallery_day, dummy_video_file):
    ...
    assert video.description == ""  # nosec
