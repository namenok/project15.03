        ...
        try:
            ...
            if user_message:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        ...
                        async with session.post(
                            ...
                            if resp.status == 200:
                                ...
                                async for line in resp.content:
                                    if (
                                        ...
                                        try:
                                            ...
                                            chunk = json_line.get("message", {}).get(
                                                "content", ""
                                            )
                                ...
                                logging.error(
                                    f"Ollama API returned status code {resp.status}: {error_text}"
                                )
                                await self.send(
                                    text_data=json.dumps(
                                        {
                    ...
                    logging.error(
                        f"Unexpected error during AI interaction: {e}", exc_info=True
                    )  # exc_info=True для повного стектрейсу
                    await self.send(
                        text_data=json.dumps(
                            {"reply": "⚠️ Виникла непередбачена помилка."}
                        )
                    )
            ...
            await self.send(
                text_data=json.dumps(
                    {"reply": "⚠️ Невірний формат повідомлення від клієнта."}
                )
            )
        except Exception as e:
            ...
            await self.send(
                text_data=json.dumps(
                    {"reply": "⚠️ Виникла непередбачена помилка обробки повідомлення."}
                )
            )
