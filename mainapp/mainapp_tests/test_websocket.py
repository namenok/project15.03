...
async def test_chat_consumer_streaming_refactored():
    ...
    mock_ollama_http_client_instance.stream_post.return_value = mock_stream_post_generator()
    ...
    with patch('mainapp.consumers.OllamaHttpClient') as MockOllamaHttpClientClass:
        ...
        assert "⏳" in json.loads(typing_response)["reply"], "Не отримано індикатор набору тексту."

        response_1 = await communicator.receive_from(timeout=5)
        assert "Hello" in json.loads(response_1)["reply"], "Не отримано фрагмент 'Hello'."

        response_2 = await communicator.receive_from(timeout=5)
        assert "there" in json.loads(response_2)["reply"], "Не отримано фрагмент 'there'."

        MockOllamaHttpClientClass.assert_called_once_with(), "Конструктор OllamaHttpClient не викликано."
        ...
        mock_ollama_http_client_instance.stream_post.assert_called_once_with(
            endpoint="chat",
            json_payload={"model": "gemma:2b", "messages": expected_messages_payload, "stream": True},
            timeout=300
...
async def test_ollama_api_error_handling():
    ...
    mock_ollama_http_client_instance.stream_post.return_value = ErrorRaisingAsyncIterator()
    ...
    with patch('mainapp.consumers.OllamaHttpClient') as MockOllamaHttpClientClass:
        ...
        typing_response = await communicator.receive_from(timeout=5)
        assert "⏳" in json.loads(typing_response)["reply"], "Не отримано індикатор набору тексту перед помилкою API."
...
async def test_network_error_handling():
    ...
    with patch('mainapp.consumers.OllamaHttpClient') as MockOllamaHttpClientClass:
        ...
        await communicator.send_to(text_data=json.dumps({"message": "test network error"}))

        typing_response = await communicator.receive_from(timeout=5)
        assert "⏳" in json.loads(typing_response)["reply"], "Не отримано індикатор набору тексту перед мережева помилкою."
